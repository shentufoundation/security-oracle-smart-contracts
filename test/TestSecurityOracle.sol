...
contract TestSecurityOracle {
    ...
    uint256 constant DEFAULT_SCORE = 50;
    ...
    function testDefaultScore() public {
        ...
        Assert.equal(
            score,
            DEFAULT_SCORE,
            "initial default score wasn't expected"
        );
    }
    ...
    function testGetSecurityScoreStringParameter() public {
        uint256 score =
            uint256(so.getSecurityScore(msg.sender, "getPrice(string)"));
    ...
    function testGetSecurityScores() public {
        ...
        uint8[] memory scores =
            so.getSecurityScores(addresses, functionSignatures);
